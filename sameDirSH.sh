...
services:
  $projectname:
    build: .
    image: $username/$projectname:local" > docker-compose.yml
...
# excecute redis-cli
docker exec -it "'$redisid'" redis-cli" > redis-script.sh
...
docker exec -it "'$dbid'" psql dbname -U dbuser" > postgres-script.sh
...
# save container id to txt file
docker inspect --format="{{.Id}}" $projectname > container_id.txt
